  ...
  if (decision.confidence < 65 || decision.action === 'hold') {
    ...
    try {
      ...
      if ((decision.action === 'buy' || decision.action === 'long') && tradeConfirmed) {
        ...
        // Use the already-validated contract from pre-buy triple-check (GeckoTerminal exact + Alchemy symbol)
        const resolvedContract = resolvedDecision.contractAddress || null;
        console.log(`[contract] Recording position with validated contract: ${resolvedContract?.slice(0,10)}…`);

...
const TOP_N      = 2;       // long top N at any time
...
// ── Template B: Vol-Flow × Price Lag (accumulation lead-lag) ─
// Wallet inflow proxy: vol_divergence (price flat + vol rising)
// When volume accumulates ahead of price → enter before price reacts
// alpha_B = vol_accel × (1 if accumulation else 0) × (1 - |z_price|/3)
function scoreB(closes, volumes, i, garchVol) {
  const volAccel = volAcceleration(volumes, i);
  const divType  = volPriceDivergence(closes, volumes, i);
  const volZ     = volSurpriseZ(volumes, i);
  if (volAccel === null || volZ === null) return null;
  if (divType !== 'ACCUMULATION') return 0;  // only enter on confirmed accumulation

  // Volume accelerating into flat/down price = smart money loading
  const accelNorm = Math.tanh(volAccel * 2);
  const volSigNorm = Math.tanh(volZ * 0.7);

  // Penalise if price already ran up (late signal)
  const r5d = i >= 5*24 ? (closes[i]-closes[i-5*24])/closes[i-5*24] : 0;
  const latePenalty = Math.max(0, 1 - Math.abs(r5d) * 5);

  return Math.max(0, accelNorm * volSigNorm * latePenalty);
}
...
// ── Template D: OU Panic Mean Reversion ──────────────────────
// Entry: extreme z-score (panic down) + volume spike (forced selling)
// OU process tells us: how fast will it revert? Is it worth entering?
// Only enter if half-life < 72h (won't stay suppressed too long)
function scoreD(closes, volumes, i, ouParams) {
  if (!ouParams || i < 30*24) return null;
  const { halfLife, kappa } = ouParams;

  // Only trade if mean-reverts fast enough (< 3 days)
  if (halfLife > 72) return 0;

  const volZ = volSurpriseZ(volumes, i);
  if (volZ === null) return null;

  // Z-score of 1d return (panic detection)
  const win=30*24;
  if (i < win) return null;
  const r1d = (closes[i]-closes[i-24])/closes[i-24];
  const r1d_series = [];
  for (let k=i-win+1;k<=i;k++) r1d_series.push((closes[k]-closes[k-24])/closes[k-24]);
  const r_mean = r1d_series.reduce((a,b)=>a+b,0)/r1d_series.length;
  const r_std  = Math.sqrt(r1d_series.reduce((s,v)=>s+(v-r_mean)**2,0)/r1d_series.length);
  const zReturn = r_std>0 ? (r1d-r_mean)/r_std : 0;

  // Entry condition: panic (z < -2.0) + volume spike (volZ > 1.5)
  // This is "liquidation + capitulation" — smart money buying zone
  if (zReturn > -2.0) return 0;   // not panicky enough
  if (volZ < 1.5) return 0;       // no volume confirmation — might keep falling

  // Signal strength: deeper panic + stronger volume = higher score
  const panicStrength  = Math.min(Math.abs(zReturn) - 2.0, 3.0) / 3.0;  // 0-1
  const volumeStrength = Math.min(volZ - 1.5, 3.0) / 3.0;               // 0-1
  const reversionSpeed = Math.min(1.0, 1.0 / (halfLife / 24));           // faster = better

  return panicStrength * volumeStrength * reversionSpeed;
}
...
function precomputeAll(allBars, allCloses, btcCloses) {
  ...
  console.log('  [5/5] Liquidity scores + token factors...');
...
class MetaAllocator {
  ...
  update(regime) {
    // Regime-based priors (override when signal is clear)
    if (regime) {
      if (regime.state === 'BULL' && !regime.highVol) {
        this.weights = { A: 0.45, B: 0.30, C: 0.20, D: 0.05 };
      } else if (regime.state === 'RANGE') {
        this.weights = { A: 0.15, B: 0.20, C: 0.25, D: 0.40 };
      } else if (regime.state === 'BEAR') {
        this.weights = { A: 0.0, B: 0.0, C: 0.0, D: 0.0 };  // all flat
      } else if (regime.highVol) {
        // High vol: shrink everything
...
function simulate(allBars, allCloses, symbols, tokenF, regimes, splitIdx) {
  ...
  for (let i = WARMUP; i < n; i++) {
    ...
    for (let tidx=0; tidx<allCloses.length; tidx++) {
      ...
      const sB = scoreB(f.closes, f.volumes, i, gS);
      const sC = scoreC_raw(f.closes, f.volumes, i);
      const sD = scoreD(f.closes, f.volumes, i, ou);

      if (sA !== null) scoresByStrategy.A.push({ tidx, score: sA });
    ...
    for (const entry of entryQueue) {
      ...
      const stopMult = strategy === 'D' ? 1.5 : 2.5;
      const tpMult   = strategy === 'D' ? 2.5 : 5.0;
